...
use git_ai::metrics::{
    AgentUsageValues, EventAttributes, MetricEvent, PosEncoded, SessionEventValues,
};
...
fn daemon_marks_repository_filtered_session_events_delivered_without_uploading_them() {
    ...
    repo.patch_git_ai_config(|patch| {
        patch.allow_repositories = Some(vec!["https://github.com/acme/*".to_string()]);
        patch.exclude_repositories = Some(vec!["git@github.com:acme/private".to_string()]);
    });

    let session_event = |trace_id: &str, repo_url: Option<&str>| {
    ...
    let non_session_event = MetricEvent::from_values(
        AgentUsageValues::new(),
        EventAttributes::with_version("test")
            .repo_url("https://github.com/acme/private")
            .trace_id("non-session")
            .to_sparse(),
    );
    let events = [
        session_event("allowed-session", Some("https://github.com/acme/public")),
        session_event("excluded-session", Some("https://github.com/acme/private")),
        session_event(
            "outside-allowlist-session",
            Some("https://github.com/other/repo"),
        ),
        session_event("unknown-repo-session", None),
        non_session_event,
    ];
    ...
    let uploaded_trace_ids: Vec<String> = mock_api
        .collect_requests()
        .into_iter()
        .filter(|request| request["path"].as_str() == Some("/worker/metrics/upload"))
        .flat_map(|request| {
            request["body"]["events"]
                .as_array()
                .cloned()
        ...
        .filter_map(|event| event["a"]["25"].as_str().map(str::to_string))
        .collect();

    assert!(uploaded_trace_ids.contains(&"allowed-session".to_string()));
    assert!(uploaded_trace_ids.contains(&"non-session".to_string()));
    assert!(!uploaded_trace_ids.contains(&"excluded-session".to_string()));
    assert!(!uploaded_trace_ids.contains(&"outside-allowlist-session".to_string()));
    assert!(!uploaded_trace_ids.contains(&"unknown-repo-session".to_string()));

    let metrics_db = MetricsDatabase::open_at_path(&metrics_db_path).unwrap();
    let status = metrics_db.status().unwrap();
    assert_eq!(status.total, 5);
    assert_eq!(status.delivered, 5);
